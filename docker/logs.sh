...
# 显示帮助信息
show_help() {
    echo -e "${GREEN}Network Bot 日志查看工具${NC}"
    echo ""
    echo "用法: $0 [选项]"
    echo ""
    echo "选项:"
    ...
    echo "  $0 app                # 列出应用日志文件"
    echo "  $0 app-tail app.log   # 实时查看应用日志"
    echo "  $0 error              # 查看错误日志"
    echo "  $0 search \"Claude\"    # 搜索包含 Claude 的日志"
    echo ""
}
...
tail_app_log() {
    ...
    if [ -z "$logfile" ]; then
        echo -e "${RED}错误: 请指定日志文件名${NC}"
        echo -e "${YELLOW}提示: 先使用 '$0 app' 查看可用的日志文件${NC}"
        exit 1
    fi

    local logpath="${LOGS_DIR}/${logfile}"

    if [ ! -f "${logpath}" ]; then
        echo -e "${RED}错误: 日志文件不存在: ${logpath}${NC}"
        exit 1
    fi

    echo -e "${GREEN}实时跟踪应用日志 (Ctrl+C 退出)${NC}"
...
show_error_logs() {
    ...
    echo ""
    docker logs "${CONTAINER_NAME}" 2>&1 | grep -iE "error|exception|fail|warn" --color=always
}
